...
中间落盘消除。本轮与第二轮（向量化）叠加后，6 case 从基线的 513~855 GB/s 提升到 2900~3460 GB/s（rms 路径 3320~3455，FP8 路径 2900~3140，细分见 §4.6 累计表）；FP8 路径的两次 GM 往返消失。
...
与第一轮叠加，中段状态（流水加深前）rms 路径达到 3320~3455 GB/s（83%~86% 利用率）、FP8 路径 2900~3140 GB/s，FP8 npc=32 因尾段指令开销暂时落后（详见 §4.4）。

### 4.4 第三轮：FP8 量化尾段演进（含两次主动回退）
...
3. **packed ue8m0 原生化**：kernel 内直接产出 Ascend native 的 int16/pack2 scale（biased exponent 字节按打包写指令落盘），host 侧只做视图转换——中间试过对齐 GPU pack4 布局的方案，与仓内量化链路契约不符，**推倒重来**；
4. **三段式 epilogue 定型**：amax 收集曾尝试融合进 pass 2 主循环以省一遍扫描，实测收益不抵 UB 压力，**整体回退**，保留 phase-major 三段式（每段覆盖所有行、每 tile 一次屏障）。

两次"试错后带数据回退"都把否决原因写进了提交记录——回退不是失败，是把"不行"变成可查的结论。

#### 结果

FP8 npc=128 路径与 npc=32 路径的尾段指令占比大幅下降；scale 与参考实现逐位一致（含 round_sf 指数推导与 1e-4 clamp）。npc=32 与 npc=128 的残余差距（§4.6）经指令账目归因确认为融合 norm 的固有成本（两遍 reconstruct 占向量指令 ~62%），不再投入。
...
第一次"任务重划 + 动态范围 2D 搬运"的尝试因动态范围在后端不安全被**整体回退**，分析清约束后以"整块静态搬运 + 逐行回退"重做——同一目标，两种形态，后者存活至今。
...
| 版本 | rms 4096+res | rms 7168+res | rms 7168 | fp8 4096/128 | fp8 7168/128 | fp8 7168/32 |
|---|---:|---:|---:|---:|---:|---:|
| 接手基线 (us) | 755.9 | 1099.4 | 662.3 | 919.4 | 1385.7 | 1264.2 |
| + 融合/向量化/FP8 尾段/2D 搬运 | 159.0 | 272.0 | 141.6 | 151.8 | 264.0 | 291.5 |
| + 流水加深（最终） | **160.4** | **268.8** | **140.5** | **151.5** | **241.8** | **291.7** |
| 累计加速 | ×4.7 | ×4.1 | ×4.7 | ×6.1 | ×5.7 | ×4.3 |

![三组里程碑的时延演进（归一化到接手基线）](norm_latency_evolution.png)

收益结构：融合 + 向量化贡献了累计加速的大头（4.0~6.1 倍），流水加深在已高水位上再拿 0~8.2%——四轮解决的是性质不同的瓶颈（落盘往返 / 指令效率 / 存储布局 / 流水节拍）。
...
**停止线**：MTE2（瓶颈管线）剩余空闲为流水填充/排空与 DRAM 物理效率；npc=32 的向量差距经指令账目归因为融合 norm 的固有成本（两遍扫描占向量指令 ~62%）；上表六个方向均已否决。**没有遗留的"零复杂度 + 可靠收益"候选**——继续压榨需要动结构，性价比不再成立。
